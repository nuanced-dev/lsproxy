...
import { SERVICE_DIGEST, BUILDER_DIGEST, LABEL_NUANCED_ROLE, BASE_DIGEST, NUANCED_ROLE_BASE, SOURCE_DIGEST, NUANCED_ROLE_BUILDER, NUANCED_ROLE_SOURCE, NUANCED_ROLE_SERVICE, SOURCE_ARCHIVE, SOURCE_DIR } from './util/constants';
import { execOrThrow, findSnapshotByDigest, startInstance } from './util/morphcloud';
...
async function ensureBuilder(client: MorphCloudClient, minimalSnapshot: Snapshot): Promise<Snapshot> {
    ...
    try {
        ...
        await execOrThrow(builderInstance, `
            apt-get update -qq \
        ...
        `, { verbose: VERBOSE, prefix: "  | " });
        const dockerVersion = await execOrThrow(builderInstance, 'docker --version');
...
async function ensureSource(client: MorphCloudClient, builderSnapshot: Snapshot): Promise<Snapshot> {
    ...
    try {
        ...
        await execOrThrow(sourceInstance, `
...
async function ensureService(client: MorphCloudClient, sourceSnapshot: Snapshot): Promise<Snapshot> {
    ...
    try {
        ...
        await execOrThrow(serviceInstance, `
            cd ${SOURCE_DIR} \
...
async function main() {
    ...
    try {
        const minimalSnapshot = await ensureBase(client);
        const builderSnapshot = await ensureBuilder(client, minimalSnapshot);
        const sourceSnapshot = await ensureSource(client, builderSnapshot);
        const baseSnapshot = await ensureService(client, sourceSnapshot);
        process.exit(0);
    } catch (error) {
