...
import { getLocallyUniqueDigest, getGloballyUniqueDigest } from "./digest";
...
export async function ensureInstanceConfigDirectory(instance: Instance): Promise<string> {
...
export async function ensureWorkspaceConfigDirectory(workspaceDigest: string): Promise<string> {
    const configDir = await ensureConfigDirectory();
...
export async function getWorkspaceProcessRcPath(workspaceDigest: string): Promise<string> {
