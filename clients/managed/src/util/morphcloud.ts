...
export async function findSnapshotByDigest(client: MorphCloudClient, digest: string): Promise<Snapshot | undefined> {
...
export async function bootInstance(client: MorphCloudClient, snapshot: Snapshot, opts?: {
    vcpus?: number;
    memory?: number;
    diskSize?: number;
...
export async function startInstance(client: MorphCloudClient, snapshot: Snapshot, opts?: {
    metadata?: Record<string, string>;
}): Promise<Instance> {
...
export async function findInstance(client: MorphCloudClient, opts?: {
    metadata?: Record<string, string>;
    ensureReady?: boolean;
}): Promise<Instance> {
...
export async function listInstances(client: MorphCloudClient, opts?: {
    metadata?: Record<string, string>;
}): Promise<Instance[]> {
    const listOpts: InstanceListOptions = {
        ...opts,
...
export function formatInstanceStatus(status: InstanceStatus): string {
    switch (status) {
        ...
        case InstanceStatus.PENDING: return "PENDING";
        case InstanceStatus.READY: return "READY";
        case InstanceStatus.PAUSED: return "PAUSED";
        case InstanceStatus.SAVING: return "SAVING";
    }
}

export async function execOrThrow(instance: Instance, cmd: string, opts?: {
    verbose?: boolean,
    prefix?: string,
}): Promise<string> {
    const {
