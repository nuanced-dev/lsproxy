...
import { ensureConfigDirectory, ensureInstanceConfigDirectory } from "./config";
...
export class MutagenClient {
    ...
    async findSync(opts?: { ensureReady?: boolean }): Promise<boolean> {
        try {
    ...
    private async spawnMutagen(args: string[], opts?: { env?: Record<string, string> }): Promise<void> {
...
function getMutagenBinaryPath(): string {
    ...
    const binPath = join(__dirname, "..", "..", "bin", "mutagen", platformArch, binaryName);
