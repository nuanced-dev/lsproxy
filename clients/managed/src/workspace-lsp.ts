...
import { SERVICE_DIGEST, NUANCED_ROLE_WORKSPACE, LABEL_NUANCED_ROLE, LABEL_NUANCED_WORKSPACE_DIGEST } from './util/constants';
import { findSnapshotByDigest, startInstance, findInstance, formatInstanceStatus } from './util/morphcloud';
...
async function main() {
    ...
    try {
        await processRc.acquire(async () => {
            workspaceInstance = await findInstance(client, { metadata, ensureReady: true });

            if (!workspaceInstance) {
                console.error("Creating workspace instance...");
                const serviceSnapshot = await findSnapshotByDigest(client, SERVICE_DIGEST);
                if (!serviceSnapshot) {
                    throw new Error(`Missing service snapshot. Run setup.ts first.`);
                }
                workspaceInstance = await startInstance(client, serviceSnapshot, { metadata });
            }
        ...
        const args = [
            "server",
            "--proxy-image", "nuanced-lsp-proxy:latest",
            "--watchdog-image", "nuanced-lsp-watchdog:latest",
            "--wrapper-image", "nuanced-lsp-wrapper:latest",
            "--shared",
            "workspace",
        ];
            ...
            await processRc.release(async () => {
                ...
                if (workspaceInstance) {
                    await workspaceInstance.pause().catch((e) => {
                        console.error(`Error: Cannot pause instance ${workspaceInstance!.id}: ${e}`);
                    });
                }
            });
            process.exit(0);
        ...
        await processRc.release(async () => {
            ...
            if (workspaceInstance) {
                await workspaceInstance.stop().catch((e) => {
                    console.error(`Error: Cannot stop instance ${workspaceInstance!.id}: ${e}`);
                });
            }
        });
