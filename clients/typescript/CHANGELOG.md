...
- Shared mode for the `server` command has been replaced by a flag to
  specify a container name. If the container name is set, the server
  uses the preexisting container with that name. The regular `up` and
  `down` commands can be used to manage that container.
