...
import { DEFAULT_BIND_HOST, DEFAULT_CONTAINER_NAME, DEFAULT_CONTAINER_PORT, DEFAULT_HOST_PORT, DEFAULT_MOUNT_DIR, DEFAULT_PROXY_IMAGE, DEFAULT_TIMEOUT_SECS, DEFAULT_WATCHDOG_IMAGE, DEFAULT_WRAPPER_IMAGE, } from "./defaults.js";
...
function dockerCmd(sudo, args) {
    return sudo
        ? { cmd: "sudo", fullArgs: ["docker", ...args] }
        : { cmd: "docker", fullArgs: args };
}
            ...
            if (res.ok) {
                ...
                for (const line of lines) {
                    ...
                    if (m) {
                        ...
                        if (!Number.isNaN(t))
                            since = new Date(t + 1); // advance past this line
                    }
                    else {
...
// Lifecycle commands
// ───────────────────────────────────────────────────────────────────────────────
export async function up(workspace, opts) {
    const { containerName = DEFAULT_CONTAINER_NAME, languageContainerVersion = LANGUAGE_CONTAINER_VERSION, proxyImage: proxyImage = DEFAULT_PROXY_IMAGE, watchdogImage: watchdogImage = DEFAULT_WATCHDOG_IMAGE, wrapperImage: wrapperImage = DEFAULT_WRAPPER_IMAGE, timeout = DEFAULT_TIMEOUT_SECS, sudo = false, stream = false, ro = false, bindHost = DEFAULT_BIND_HOST, debug = false, } = opts;
    let { hostPort = DEFAULT_HOST_PORT } = opts;
    if (!Number.isInteger(hostPort) || hostPort < 0) {
        throw new Error("up(): hostPort is required and must be a positive integer >= 0");
    }
    if (workspace.trim() === "") {
        throw new Error("Invalid argument: 'workspace' is required and cannot be empty.");
    }
    ...
    if (hostPort === 0) {
        const portRes = runDockerCmd(["port", containerName, `${DEFAULT_CONTAINER_PORT}/tcp`], { sudo });
        if (!portRes.ok) {
            return err({
    ...
    let stopLogs = null;
    if (stream)
        stopLogs = pollLogs(containerName, sudo, 500);
    try {
        if (timeout > 0) {
            ...
            const h = await pollHttpWithRetries("GET", "/v1/system/health", base, {
...
export async function down(containerName, sudo) {
...
export async function port(containerName, containerPort, sudo) {
...
export async function logs(containerName, sudo, opts) {
    const args = ["logs"];
    if (opts?.stream)
        args.push("-f");
...
export async function run(script, containerName, sudo, stream, env, envFile) {
    ...
    let r = runDockerCmd(["exec", containerName, "sh", "-lc", `mkdir -p ${containerTmpDir}`], { sudo, stream });
    if (!r.ok) {
        return err({
            message: dockerFailureMessage(r),
            error_code: r.code,
    ...
    // Copy script.
    r = runDockerCmd(["cp", hostScriptPath, `${containerName}:${containerScriptPath}`], { sudo, stream });
    if (!r.ok) {
        return err({
            message: dockerFailureMessage(r),
            error_code: r.code,
    ...
    if (Array.isArray(env)) {
        for (const e of env) {
            if (e && String(e).trim())
                execArgs.push("-e", String(e));
        }
    }
    execArgs.push(containerName, "sh", "-lc", `chmod +x ${containerScriptPath} || true; ${containerScriptPath}`);
    ...
    // Clean up the script.
    r = runDockerCmd(["exec", containerName, "sh", "-lc", `rm -f ${containerScriptPath}`], { sudo, stream });
    if (!r.ok) {
        return err({
            message: dockerFailureMessage(r),
            error_code: r.code,
...
function dockerInspectField(containerName, fieldTemplate, // e.g. "{{.State.Running}}"
sudo) {
    return runDockerCmd(["inspect", "-f", fieldTemplate, containerName], {
        sudo,
    });
}
export async function status(containerName, sudo) {
    const runningRes = dockerInspectField(containerName, "{{.State.Running}}", sudo);
    if (!runningRes.ok) {
        return err({
    ...
    if (!running) {
        ...
        const exitRes = dockerInspectField(containerName, "{{.State.ExitCode}}", sudo);
        const errorRes = dockerInspectField(containerName, "{{.State.Error}}", sudo);
        ...
        if (typeof exitCode === "number")
            msgParts.push(`ExitCode=${exitCode}.`);
        if (dockerError)
            msgParts.push(`Error="${dockerError}".`);
        return err({
    ...
    const psRes = runDockerCmd(["ps", "--filter", `name=${containerName}`, "--format", "{{.Status}}"], { sudo });
    if (!psRes.ok) {
        return err({
...
export async function pull(image, sudo, stream) {
