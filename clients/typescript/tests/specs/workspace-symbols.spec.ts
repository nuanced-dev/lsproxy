...
describeDocker("TypeScript workspace + symbols", () => {
  for (const language of LANGUAGES) {
    describe(language.label, () => {
      ...
      it("reads source files from the workspace", async () => {
        ...
        expect((sourceRecord.source_code as string).length).toBeGreaterThan(0);
      ...
      it("finds definitions for identifiers", async () => {
        ...
        for (const file of files) {
          ...
          for (const entry of entries) {
            ...
            const payload = await runner.findDefinition(file, line, character);
