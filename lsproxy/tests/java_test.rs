/// Integration test for Java language support in the new containerized architecture.
///
/// This test:
/// - Starts the LSProxy service in-process (not in Docker)
/// - The service spawns a Java language container on-demand
/// - Tests workspace and symbol endpoints
/// - Requires: lsproxy-java:latest Docker image
...
fn test_server_integration_java() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Test definitions-in-file endpoint (requires Java language container)
